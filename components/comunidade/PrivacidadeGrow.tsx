...
import {
  Lock, Users, Home, Plane, Clapperboard, BookOpen, ShieldCheck, Info,
} from 'lucide-react';

// =============================================================================
// Privacidade do Grow — o que o grupo enxerga.
//
// ⚠️ ISTO ERA UMA ABA PRÓPRIA (`/grow/configuracoes`, item "Compartilhamento" no
// grupo Grow da sidebar). Virou uma SEÇÃO daqui porque eram dois lugares pra
// resolver a mesma pergunta — "quem vê o quê" — e quem divide a conta já está
// nesta tela mexendo em membros e permissões. A rota antiga continua existindo
// e redireciona pra cá: ela está em link de WhatsApp e no histórico de quem já usa.
// =============================================================================
...
export default function PrivacidadeGrow() {
  const { phone } = useAuth();
  ...
  return (
    <section className="space-y-4 pt-2">
      <header>
        <h2 className="text-lg font-bold text-foreground tracking-tight">Privacidade do Sora Grow</h2>
        <p className="mt-0.5 text-sm text-muted-foreground">
          Dividir as finanças não significa dividir a rotina. Aqui você escolhe o que o grupo enxerga.
        </p>
      </header>

      {/* Sempre privado */}
      <div
        className="rounded-3xl border border-border/40 backdrop-blur-xl p-5 sm:p-6"
        style={{ background: 'hsl(var(--bg-card) / 0.5)' }}
      >
        <div className="flex items-center gap-2 mb-1">
          <Lock size={16} style={{ color: BRAND }} />
          <h3 className="text-base font-bold text-foreground">Sempre privado</h3>
        </div>
      ...
      >
        <div className="flex items-center gap-2 mb-1">
          ...
          <h3 className="text-base font-bold text-foreground">Compartilhar com o grupo</h3>
        </div>
        <p className="text-sm text-muted-foreground mb-4">
            ...
            {OPCIONAIS.map((aba, i) => {
              ...
              return (
                <div key={aba.key}
                  ...
                  {/* Estado em ÍCONE + PALAVRA, nunca só na cor do toggle. */}
