...
// ── 5. O MÊS 0 É O QUE AINDA VEM ────────────────────────────────────────────
//
// ⚠️ E O REALIZADO NÃO ENTRA, por causa da compra no cartão. Ela é transação no
// dia em que acontece (nasce `pago=true`), então está em `realizado.gastos`; e
// está TAMBÉM dentro da fatura em aberto, que é o que ainda vai sair da conta.
// Somar as duas metades conta a mesma despesa duas vezes — foi o relato:
// R$ 1.041,05 de fatura em cima de R$ 1.345,29 que já a continham.
console.log('── 5. mês 0 é o que ainda vem ──');
{
  const meses = projetarMeses({
    ...
    restante: { receitas: 1000, gastos: 76.2 },
  });
  eq(meses[0].receitaFirme, 1000, 'mês 0 mostra o que ainda entra');
  eq(meses[0].despesaFirme, 76.2, 'e o que ainda sai — o realizado fica de fora');
  eq(meses[1].receitaFirme, 1000, 'mês 1 já é projeção normal');
  eq(meses[1].despesaFirme, 100, 'idem despesa');

  // Sem `restante` não há como saber o que falta: o mês 0 vira o mês agendado
  // inteiro, que é a melhor resposta possível com o que se tem.
  const semRestante = projetarMeses({
    inicio: '2026-09', quantidade: 2, saldoInicial: 500,
    recorrencias: [{ tipo: 'Gasto', valor: 100 }],
    dividas: [], faturas: [],
    realizado: { receitas: 0, gastos: 23.8 },
  });
  eq(semRestante[0].despesaFirme, 100, 'sem restante, o mês 0 é o mês agendado');
}
console.log('  ok');
...
// ── 12. O MÊS 0 FECHA NO MESMO NÚMERO DA MANCHETE ───────────────────────────
//
// ⚠️ DOIS BUGS REAIS, um em cima do outro, e os dois eram DINHEIRO NA TELA.
//
// 1) O `saldoInicial` que se passa aqui é o saldo de HOJE, que já reflete o que
//    entrou e saiu no mês. Somar o realizado por cima dele inflava o fecho: a
//    manchete dizia R$ 52,85 e a projeção do MESMO mês dizia R$ 154,02.
// 2) Depois, ao exibir "já saiu + ainda sai", a COMPRA NO CARTÃO era contada
//    duas vezes — ela está em `realizado.gastos` (é transação no dia) e está
//    dentro da fatura em aberto (que é o que ainda sai da conta).
//
// Nos dois casos a saída é a mesma: o mês 0 responde uma pergunta de CAIXA, e
// quem manda nela é só o que ainda vai acontecer.
console.log('── 12. mês 0 fecha igual à manchete ──');
{
  const saldoHoje = 1280.89;
  // 1.345,29 de gasto já realizado — e 1.041,05 disso é compra no cartão, que
  // reaparece inteira dentro da fatura em aberto logo abaixo.
  ...
  const meses = projetarMeses({
    ...
    recorrencias: [
      ...
      { tipo: 'Gasto', valor: 514.99, dia_vencimento: 26 },
    ],
    dividas: [],
    faturas: [{ cartao_id: 'c1', nome: 'MP', restante: 1041.05, venc: '2026-09-10' }],
    realizado, restante,
  });
  ...
  eq(meses[0].saldoInicial, saldoHoje, 'e parte do saldo de hoje, não de uma reconstrução');

  // ⚠️ O TESTE QUE PEGA A CONTAGEM EM DOBRO: o total do mês 0 é o que AINDA sai,
  //    e nada além disso. Somar o realizado daria 2.901,33 — quase o dobro.
  eq(meses[0].despesaFirme + meses[0].despesaEstimada, restante.gastos,
    'despesas do mês 0 = só o que ainda sai');
  eq(meses[0].receitaFirme + meses[0].receitaEstimada, restante.receitas,
    'receitas do mês 0 = só o que ainda entra');

  // A conta fecha sozinha, sem linha de sobra pra tapar buraco.
