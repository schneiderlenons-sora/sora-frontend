...
console.log('── 7. useDinheiro/useComSimbolo renderizados ──');
{
  ...
  // `moeda` explícita: o cartão em real num grupo em dólar mostra a fatura em real.
  for (const v of VALORES) {
    eq(pegar('BRL', () => useDinheiro({ moeda: 'BRL' }))(v), painelHoje(v), `moeda BRL explícita num grupo em real ${v}`);
    for (const vazia of [undefined, null, '']) {
      eq(pegar('BRL', () => useDinheiro({ moeda: vazia }))(v), painelHoje(v), `moeda ${JSON.stringify(vazia)} = a base ${v}`);
    }
  }
  eq(pegar('USD', () => useDinheiro({ moeda: 'BRL' }))(514.35), painelHoje(514.35), '⚠️ grupo em dólar, cartão em real: a fatura sai em real');
  eq(pegar('USD', () => useDinheiro({ moeda: null }))(100), pegar('USD', () => useDinheiro())(100), 'sem moeda do cartão: a do grupo');
}
console.log('  ok');
