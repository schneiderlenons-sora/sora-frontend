...
// ── 11. A INSTRUMENTAÇÃO PRECISA SER ÚTIL E INÓCUA ─────────────────────────
//
// Depois de CINCO rodadas de correção por teoria (duas delas derrubadas por
// medição: "é a VPN" e "a rotação revoga a sessão"), o middleware passou a
// gravar o instante da anomalia. Este bloco protege as duas propriedades que
// fazem isso valer a pena.
console.log('── 11. instrumentação do logout ──');
{
  const mw = fs.readFileSync(new URL('../middleware.ts', import.meta.url), 'utf8');

  ok(/function registrarIncidente/.test(mw), 'existe o registrador do incidente');
  ok(/if \(falhaTransitoria\) \{[\s\S]{0,200}registrarIncidente/.test(mw),
    'grava exatamente no instante da anomalia (chegou com cookie, validação vazia)');

  // ⚠️ ÚTIL: a identidade sai dos cookies DA ENTRADA. Se fosse lida depois da
  //    validação, um `_removeSession` já teria esvaziado o cookie e a linha
  //    sairia sem saber de QUEM é — justo o campo que a investigação precisa.
  const iDono = mw.indexOf('const donoNaEntrada =');
  const iClient = mw.indexOf('createServerClient(');
  ok(iDono > -1 && iClient > -1 && iDono < iClient,
    '⚠️ a identidade é lida ANTES de o client do Supabase existir');

  // Sessão acima de ~4 KB vem fatiada; sem juntar os pedaços o JSON não fecha
  // e a identidade se perde justamente em quem tem sessão grande.
  ok(/sort\(\(a, b\) => a\.name\.localeCompare\(b\.name\)\)/.test(mw),
    'junta os pedaços do cookie fatiado na ordem certa');

  // ⚠️ INÓCUA: nunca decide nada, nunca estoura, e tem teto de tempo.
  ok(/new AbortController\(\)/.test(mw) && /setTimeout\(\(\) => ctrl\.abort\(\)/.test(mw),
    '⚠️ tem teto de tempo — não segura a resposta se o Supabase demorar');
  ok(/catch \{ \/\* instrumentação nunca derruba o que está medindo \*\/ \}/.test(mw),
    '⚠️ falha dela é engolida — medir não pode quebrar o que está sendo medido');
  ok(/incidentesRecentes/.test(mw),
    'tem trava de repetição (laço de redirect não vira flood)');

  // ⚠️ Não guarda token. Se um dia alguém acrescentar, isto acusa.
  const fn = mw.slice(mw.indexOf('async function registrarIncidente'));
  const corpo = fn.slice(0, fn.indexOf('\n}'));
  ok(!/access_token|refresh_token/.test(corpo),
    '⚠️ não grava access token nem refresh token');
}
console.log('  ok');
console.log('');
if (falhas.length) {
