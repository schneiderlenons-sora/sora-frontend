...
// ── 15. Dívida pela MESMA regra do card (set/2026) ──────────────────────────
//
// Relato: "a dívida não aparece no extrato previsto". Elas ficavam fora porque
// a projeção antiga reprojetava a parcela já paga. Agora a data sai de
// `proximoVencimento`, a mesma fonte do card.
console.log('── 15. dívidas no extrato ──');
{
  const de = '2026-09-18', ate = '2026-11-30';
  const div = (extra) => [{ id: 'dx', titulo: 'Celular', valor_parcela: 515, dia_vencimento: 20,
    parcelas_total: 4, parcelas_pagas: 0, ...extra }];
  const datas = (dividas) => montarExtrato({ ...base, de, ate, recorrencias: [], faturas: [], dividas })
    .dias.flatMap((d) => d.linhas).filter((l) => l.origem === 'divida').map((l) => l.data).join(',');
  eq(datas(div({})), '2026-09-20,2026-10-20,2026-11-20', 'parcela do mês + seguintes');
  eq(datas(div({ ultimo_pagamento: '2026-09-17' })), '2026-10-20,2026-11-20',
    'paga ADIANTADO no mês não é projetada de novo');
  eq(datas(div({ data_inicio: '2026-09-20' })), '2026-10-20,2026-11-20',
    '1ª parcela nunca no dia/mês da contratação (mesma regra do card)');
  eq(datas(div({ parcelas_pagas: 3 })), '2026-09-20', 'só as parcelas que faltam');
  eq(datas(div({ status: 'quitada' })), '', 'quitada não entra');
  eq(datas(div({ nos_previstos: false })), '', 'tirada dos previstos não entra');
  eq(datas([{ ...div({})[0], dia_vencimento: 31 }]).split(',')[0], '2026-09-30', 'clamp ao último dia do mês');
}
console.log('  ok');

// ── 16. Pulada aparece na lista de puladas, fora do saldo ───────────────────
console.log('── 16. puladas ──');
{
  const ex = montarExtrato({
    ...base, dividas: [], faturas: [],
    ajustes: [{ recorrenciaId: 'r1', competencia: '2026-09', status: 'pulado' }],
  });
  eq(ex.dias.flatMap((d) => d.linhas).some((l) => l.recorrenciaId === 'r1'), false, 'pulada não entra no saldo');
  eq(ex.puladas.length, 1, 'pulada vai pra lista (com volta)');
  eq(ex.puladas[0]?.data, '2026-09-10', 'com a data original');
  const semPulo = montarExtrato({ ...base, dividas: [], faturas: [] });
  eq(semPulo.puladas.length, 0, 'sem ajuste, lista vazia');
  const filtrado = montarExtrato({
    ...base, dividas: [], faturas: [], carteiras: ['Outra'],
    ajustes: [{ recorrenciaId: 'r1', competencia: '2026-09', status: 'pulado' }],
  });
  eq(filtrado.puladas.length, 0, 'filtro de conta vale pras puladas também');
}
console.log('  ok');
