...
import {
  ...
  cicloMesCalendario, dentroDoCiclo, pertenceAFatura, criterioDaFatura, AGRUPA_POR_BANCO,
...
// ── 7B. HÍBRIDO: banco onde ele opinou, ciclo onde ele calou ──────────────
//
// O modo `bill` é tudo-ou-nada e por isso quase nunca podia ser usado: só
// ~14% das linhas chegam com `of_bill_id` (o emissor só vincula depois do
// fechamento). O híbrido deixa as duas fontes conviverem sem se misturar — a
// linha COM vínculo obedece ao banco, a SEM vínculo obedece ao ciclo. Não há
// sobreposição: ou a linha tem `of_bill_id`, ou não tem.
//
// Medido em 24 faturas com histórico completo antes de ligar: 9 aproximam do
// total do banco, 12 ficam iguais, 3 pioram (as 3 num cartão que não fecha em
// configuração nenhuma — nem hoje, nem híbrido, nem ciclo deslocado).
{
  ...
  const ago = cicloPorCompetencia(cartao, '2026-08');
  const BILL = 'bill-ago';
  const tx = (bill, data) => ({ of_bill_id: bill, data });

  casos.push([pertenceAFatura(tx(BILL, '2026-06-20'), cartao, ago, true, 'hibrido', BILL),
    'linha vinculada a esta fatura entra, mesmo datada FORA do ciclo']);
  casos.push([!pertenceAFatura(tx('bill-set', '2026-08-01'), cartao, ago, true, 'hibrido', BILL),
    'linha vinculada a OUTRA fatura não entra, mesmo DENTRO do ciclo']);
  casos.push([pertenceAFatura(tx(null, '2026-08-01'), cartao, ago, true, 'hibrido', BILL),
    'linha SEM vínculo dentro do ciclo entra (é a fatura ainda aberta)']);
  casos.push([!pertenceAFatura(tx(null, '2026-06-20'), cartao, ago, true, 'hibrido', BILL),
    'linha SEM vínculo fora do ciclo não entra']);
  // ⚠️ Sem saber a fatura do emissor o híbrido NÃO pode virar peneira: cai no
  // ciclo, que é o comportamento de sempre.
  casos.push([pertenceAFatura(tx('bill-x', '2026-08-01'), cartao, ago, true, 'hibrido', null),
    'sem billId o híbrido cai no ciclo e não descarta nada']);
  casos.push([criterioDaFatura([], cartao, true, ago, 'bill-ago') === 'hibrido',
    'sabendo a fatura do emissor, o critério é o híbrido']);
  casos.push([criterioDaFatura([], cartao, true, ago, null) === 'ciclo',
    'sem a fatura do emissor, segue no ciclo']);
  // Chave de rollback: NEXT_PUBLIC_FATURA_AGRUPA_BANCO=0 volta tudo ao antigo.
  casos.push([AGRUPA_POR_BANCO === true, 'ligado por padrão']);

  // A soma com as duas fontes juntas, sem contar nada duas vezes.
  {
    const cartaoR = { dia_fechamento: 7, dia_vencimento: 14 };
    const cicloAgo = cicloPorCompetencia(cartaoR, '2026-08');
    const txs = [
      { data: '2026-06-14', valor: 100, of_bill_id: 'bill-ago' },  // banco: é desta
      { data: '2026-06-14', valor: 999, of_bill_id: 'bill-jul' },  // banco: é de outra
      { data: '2026-07-29', valor: 50 },                           // sem vínculo, no ciclo
      { data: '2026-06-20', valor: 77 },                           // sem vínculo, fora
    ];
    const crit = criterioDaFatura(txs, cartaoR, true, cicloAgo, 'bill-ago');
    const soma = txs.filter((t) => pertenceAFatura(t, cartaoR, cicloAgo, true, crit, 'bill-ago'))
      .reduce((s, t) => s + t.valor, 0);
    casos.push([crit === 'hibrido', 'critério é híbrido']);
    casos.push([Math.abs(soma - 150) < 0.01,
      `soma = vinculada (100) + sem vínculo no ciclo (50) = 150, deu ${soma}`]);
  }

  for (const [ok, msg] of casos) if (!ok) falhas.push(`híbrido: ${msg}`);
  console.log(`  pertenceAFatura híbrido (agrupamento do emissor): ${casos.length} casos`);
}
