...
import {
  ...
  cicloMesCalendario, dentroDoCiclo, pertenceAFatura,
...
// é o `of_bill_id` do emissor (migration 101).
{
  ...
  const parcela = (bill, v) => ({ data: '2026-06-20', valor: v, of_bill_id: bill });
  const casos = [
    [!pertenceAFatura(parcela('bill-jul', 56.66), cartao, ago, true), 'parcela 1/3 (julho) não pode entrar na fatura de agosto'],
    [pertenceAFatura(parcela('bill-ago', 56.66), cartao, ago, true), 'parcela 2/3 tem de entrar, mesmo com data de 20/06'],
    [!pertenceAFatura(parcela('bill-set', 56.67), cartao, ago, true), 'parcela 3/3 (setembro) não pode entrar'],
    [pertenceAFatura({ data: '2026-07-29', valor: 10 }, cartao, ago, true), 'sem of_bill_id: vale o ciclo (comportamento de sempre)'],
    [!pertenceAFatura({ data: '2026-06-20', valor: 10 }, cartao, ago, true), 'sem of_bill_id e fora do ciclo: fica fora'],
    [pertenceAFatura(parcela('bill-jul', 56.66), cartao, jul, false), 'fatura anterior (bill desconhecido) volta pro ciclo'],
    [!pertenceAFatura(parcela('bill-ago', 56.66), { dia_fechamento: 12, dia_vencimento: 17 }, ago, true),
      'cartão MANUAL ignora of_bill_id e usa só o ciclo'],
  ];
  for (const [ok, msg] of casos) if (!ok) falhas.push(`pertenceAFatura: ${msg}`);
  console.log(`  pertenceAFatura (parcelamento OF): ${casos.length} casos`);
}
