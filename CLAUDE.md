...
- **⚠️ `animate-fade-in` DEIXA UM `transform` PERMANENTE — e isso cria stacking context.** O keyframe `fade-in` usa `transform: translateY(...)` e o atalho é declarado com `animation-fill-mode: both`, então o `transform: translateY(0)` do estado final **fica aplicado pra sempre**. Transform não-none cria *stacking context*: qualquer filho `absolute` com z-index fica **preso dentro daquele elemento** e não alcança os irmãos de fora. Bug real (set/2026, relato de iPad): na aba Categorias, os itens "Definir limite", "Nova subcategoria" e "Excluir" do menu ⋮ **não recebiam toque nenhum** — a linha tinha `animate-fade-in`, o menu `absolute z-30` ficava preso nela, e as linhas SEGUINTES (que pintam depois no DOM) cobriam o menu e engoliam o toque. Só "Editar categoria" (o 1º item, ainda sobre a área da própria linha) funcionava — exatamente os 3 que o cliente listou. **Provado numa bancada com Playwright**: `elementFromPoint` no centro de cada item devolvia a linha de baixo; removendo só a animação, os 4 voltavam. ⚠️ **z-index NÃO resolve** — dentro de um stacking context ele não alcança quem está fora. As duas saídas: **portal pro body** (menu ⋮, ancorado na posição medida do botão — resolve junto o `overflow-hidden` do card da lista, que cortava o menu da ÚLTIMA linha) ou **elevar o elemento que TEM o transform** (`relative z-30` na linha enquanto o seletor de cor está aberto). Ao criar dropdown/popover dentro de linha animada, já nasça por portal.
