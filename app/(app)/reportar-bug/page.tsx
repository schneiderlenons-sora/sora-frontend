...
import Novidades from '@/components/suporte/Novidades';
import {
  Bug, ImagePlus, X, Loader2, CheckCircle2, AlertCircle, Send, LifeBuoy, Lightbulb, Sparkles,
} from 'lucide-react';

const BRAND = 'hsl(var(--primary))';
const MAX_IMG = 6 * 1024 * 1024; // 6MB (cabe no limite de 10MB do backend após base64)

type Aba = 'novidades' | 'problema' | 'melhoria';

const ABAS: readonly [Aba, string, typeof Sparkles][] = [
  ['novidades', 'Novidades',           Sparkles],
  ['problema',  'Relatar um problema', Bug],
  ['melhoria',  'Propor melhoria',     Lightbulb],
];

export default function ReportarBugPage() {
  // NOVIDADES É A PORTA DE ENTRADA — é a pergunta que trouxe a pessoa aqui
  // ("como fico sabendo que algo mudou no app?"); relatar/propor são o que
  // ela pode fazer depois de ver. O item "Novidades e sugestões" da sidebar
  // linka pra cá sem `?aba=`, então o padrão tem de ser este.
  const [aba, setAba] = useState<Aba>('novidades');
  const ehMelhoria = aba === 'melhoria';

  // ── ABA VINDA DA URL ────────────────────────────────────────────────────
  // ⚠️ EFEITO, não valor inicial do useState: ler a URL no primeiro render
  // divergiria do HTML do servidor (hydration mismatch) — mesmo padrão do
  // Extrato em PrevistosClient.tsx.
  useEffect(() => {
    if (typeof window === 'undefined') return;
    const p = new URLSearchParams(window.location.search);
    const v = p.get('aba');
    if (v === 'problema' || v === 'melhoria' || v === 'novidades') setAba(v);
    else if (v === 'sugestao') setAba('melhoria'); // link antigo (?aba=sugestao)
  }, []);
  ...
  // Trocar de aba troca de "relato" — sem isso, ir de um relato JÁ ENVIADO
  // pra outra aba mostraria a tela de sucesso do relato errado.
  ...
  async function enviar() {
    ...
    try {
      await api.bug.reportar({ mensagem: mensagem.trim(), imagem: imagem || undefined, tipo: ehMelhoria ? 'melhoria' : 'problema' });
      setEnviado(true);
  ...
  return (
    <>
      <div className="max-w-2xl mx-auto px-4 pb-24 space-y-6">
        ...
        {/* ── Abas: Novidades · Relatar um problema · Propor melhoria ─────── */}
        ...
        {aba === 'novidades' ? (
          <>
            <div className="space-y-3 pt-1">
              ...
              <p className="text-sm text-muted-foreground leading-relaxed">
                Tudo que já mudou na Sora, na ordem em que foi lançado.
              </p>
            </div>
            <Novidades />
          </>
          ...
          <>
            ...
            {/* Chamados abertos — a conversa com o suporte. Fica ABAIXO do
                ...
                e consulta, nao a acao principal. So os do tipo desta aba — senao
                "Relatar um problema" mostraria sugestao de melhoria junto.
                Some sozinha quando nao ha chamado nenhum (o componente devolve
                null). */}
            <MeusChamados recarregarRef={recarregarChamados} tipoFiltro={aba} />
